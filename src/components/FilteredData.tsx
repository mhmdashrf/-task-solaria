...
function FilteredData({ data }: { data: Unit[] }) {
  ...
  const newObj: { [key: number]: Unit } = {};
  console.log("filter", data);
  console.log("kk", newObj);
  data.forEach((item) => {
    newObj[item.code] = item;
  });
  const handleColorFill = (code: number) => {
    const status = newObj[code]?.status;
    switch (status) {
      case "sold":
        return "red";
  ...
  const updateSvg = (svg: SVGSVGElement | null) => {
    ...
    Array.from(polygons).forEach((polygon) => {
      ...
      if (code) {
        polygon.setAttribute("fill", handleColorFill(Number(code)));
        ...
        polygon.onmouseleave = () => {
          setResult({ ...result, data: null, show: false });
        };
      }
    });
  };
